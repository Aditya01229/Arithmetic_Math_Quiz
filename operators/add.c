...
int addlevel1(int *p1, int *p2)
{
    for (int i = 0; i < 10; i++)
    {
        int temp1 = printRandoms(10, 99);
        int temp2 = printRandoms(100, 999);
        *(p1 + i) = temp1;
        *(p2 + i) = temp2;
    }
}
...
int printadd(int lvl)
{
    ...
    FILE *ptr = NULL;
    FILE *temp = NULL;
    temp = fopen("files/add/temp.txt", "w");
    fputs(name, temp);
    fprintf(temp, " %d %.3f\n", (crt * 10), (total_time / 1000));
    if (lvl == 1)
    {
        ptr = fopen("files/add/add1.txt", "r");
    }
    else if (lvl == 2)
    {
        ptr = fopen("files/add/add2.txt", "r");
    }
    else if (lvl == 3)
    {
        ptr = fopen("files/add/add3.txt", "r");
    }
    char ch;
    ...
    if (lvl == 1)
    {
        remove("files/add/add1.txt");
        rename("files/add/temp.txt", "files/add/add1.txt");
    }
    else if (lvl == 2)
    {
        remove("files/add/add2.txt");
        rename("files/add/temp.txt", "files/add/add2.txt");
    }
    else if (lvl == 3)
    {
        remove("files/add/add3.txt");
        rename("files/add/temp.txt", "files/add/add3.txt");
    }
...
void addreset(){
    FILE *ptr = NULL;
    ptr = fopen("files/add/add1.txt", "w");
    fclose(ptr);
    ptr = fopen("files/add/add2.txt", "w");
    fclose(ptr);
    ptr = fopen("files/add/add3.txt", "w");
    fclose(ptr);
}